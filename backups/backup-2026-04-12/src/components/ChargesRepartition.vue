  ...
  <div class="cr">
    ...
    <div class="cr__viz">
      <svg :viewBox="`0 0 ${W} ${H}`" :width="W" :height="H" style="overflow:visible;display:block;margin:auto">
        ...
        <!-- Texte P1 -->
        <text :x="tx1" :y="cy - r1 * 0.28" text-anchor="middle" :fill="C_P1" font-family="Inter,sans-serif" font-size="11" font-weight="600" opacity="0.85">{{ p1.nom }}</text>
        <text :x="tx1" :y="cy + r1 * 0.14" text-anchor="middle" :fill="C_P1_DARK" font-family="Inter,sans-serif" :font-size="pctFontSize(r1)" font-weight="700">{{ pctP1 }}%</text>
        <text :x="tx1" :y="cy + r1 * 0.44" text-anchor="middle" :fill="C_P1" font-family="Inter,sans-serif" font-size="11" font-weight="500" opacity="0.80">{{ fmt(totalMensuelP1) }}</text>

        <!-- Texte P2 -->
        <text :x="tx2" :y="cy - r2 * 0.28" text-anchor="middle" :fill="C_P2" font-family="Inter,sans-serif" font-size="11" font-weight="600" opacity="0.85">{{ p2.nom }}</text>
        <text :x="tx2" :y="cy + r2 * 0.14" text-anchor="middle" :fill="C_P2_DARK" font-family="Inter,sans-serif" :font-size="pctFontSize(r2)" font-weight="700">{{ pctP2 }}%</text>
        <text :x="tx2" :y="cy + r2 * 0.44" text-anchor="middle" :fill="C_P2" font-family="Inter,sans-serif" font-size="11" font-weight="500" opacity="0.80">{{ fmt(totalMensuelP2) }}</text>
      </svg>
    </div>

    <div class="cr__legend">
      <div class="cr__legend-row">
        <span class="cr__dot" :style="{ background: C_P1 }"></span>
        <span class="cr__legend-name">{{ p1.nom }}</span>
        <div class="cr__bar-track">
          <div class="cr__bar-fill" :style="{ width: pctP1 + '%', background: C_P1 }"></div>
        </div>
        <span class="cr__legend-pct">{{ pctP1 }}%</span>
      </div>
      <div class="cr__legend-row">
        <span class="cr__dot" :style="{ background: C_P2 }"></span>
        <span class="cr__legend-name">{{ p2.nom }}</span>
        <div class="cr__bar-track">
          <div class="cr__bar-fill" :style="{ width: pctP2 + '%', background: C_P2 }"></div>
        </div>
        <span class="cr__legend-pct">{{ pctP2 }}%</span>
      </div>
    </div>
  </div>
</template>
...
.cr__legend { display: flex; flex-direction: column; gap: 8px; }
.cr__legend-row { display: flex; align-items: center; gap: 8px; }
.cr__dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.cr__legend-name { font-size: 12px; color: var(--foreground); width: 100px; flex-shrink: 0; }
.cr__bar-track { flex: 1; height: 5px; background: var(--muted); border-radius: 99px; overflow: hidden; }
.cr__bar-fill { height: 100%; border-radius: 99px; transition: width 0.4s ease; }
.cr__legend-pct { font-size: 12px; font-weight: 600; color: var(--foreground); width: 32px; text-align: right; }
</style>
