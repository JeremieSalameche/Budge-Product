  ...
  <Teleport to="body">
    ...
    <Transition name="slide">
      <div v-if="visible" class="op-panel">
        ...
        <div v-if="depenses.length === 0" class="op-empty-state">
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21.21 15.89A10 10 0 1 1 8 2.83"/><path d="M22 12A10 10 0 0 0 12 2v10z"/></svg>
          <span class="op-empty-state-title">Aucune dépense pour le moment</span>
          <span class="op-empty-state-sub">Ajoutez vos premières dépenses pour obtenir une analyse de votre budget</span>
        </div>

        <!-- État parfait : rien à optimiser -->
        <div v-else-if="nbAlertes === 0" class="op-bravo">
          <div class="op-bravo-icon">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/>
              <polyline points="22 4 12 14.01 9 11.01"/>
            </svg>
          </div>
          <div class="op-bravo-body">
            <div class="op-bravo-title">Budget bien maîtrisé !</div>
            <div class="op-bravo-sub">Toutes vos catégories sont dans les seuils recommandés. Continuez comme ça.</div>
          </div>
        </div>

        <!-- Économies potentielles détaillées -->
        <div v-if="potentielTotal > 0" class="op-economies">
          <div class="op-eco-header">
            <span class="op-eco-title">Vous pourriez économiser</span>
            <span class="op-eco-total">{{ fmt(potentielTotal) }}/mois</span>
          </div>
        ...
        <!-- Stats rapides -->
        <div v-if="depenses.length > 0" class="op-summary">
        ...
        <div class="op-list">
          ...
          >
            ...
            <div class="op-item-body">
              ...
              <!-- Chips abonnements -->
              <div v-if="r.subs?.length" class="op-subs">
                <span v-for="s in r.subs" :key="s" class="op-sub-chip">{{ s }}</span>
              </div>
              <!-- Tips abonnements -->
              <ul v-if="r.tips?.length" class="op-tips">
                <li v-for="t in r.tips" :key="t">{{ t }}</li>
              </ul>
            ...
            <div v-if="r.potentiel > 0" class="op-saving">
              ...
              <div class="op-saving-lbl">à économiser</div>
            </div>
          </div>
...
const recommandations = computed(() => {
  ...
  parCategorie.value.forEach(cat => {
    ...
    if (cat.nom === 'Abonnements') {
      const nb = cat.items.length
      const MAX_ABO = 5
      const aboBudgetOk = pct <= seuil.max
      const aboNbOk = nb <= MAX_ABO

      const tips = []
      if (nb > MAX_ABO) tips.push(`Idéalement ≤ ${MAX_ABO} abonnements actifs en simultané`)
      if (nb > 7) tips.push('Auditez vos relevés — certains abonnements passent inaperçus')
      tips.push('Désactivez ce que vous n\'utilisez pas ce mois-ci plutôt que de résilier')
      if (nb > 4) tips.push('Alternez vos abonnements streaming au lieu de tout garder en même temps')

      const type = !aboBudgetOk ? (pct > seuil.max * 1.5 ? 'danger' : 'warning') : (!aboNbOk ? 'warning' : 'ok')
      const potentiel = !aboBudgetOk ? Math.round(cat.montant - total * seuil.max / 100) : 0

      let titre = `Abonnements — ${nb} actif${nb > 1 ? 's' : ''}`
      if (!aboBudgetOk) titre += ` · ${pct}% du budget`

      let desc = ''
      if (!aboBudgetOk) desc = `Seuil recommandé : ${seuil.max}%. Vous dépensez ${fmt(cat.montant)}/mois, soit ${fmt(potentiel)} de plus que conseillé.`
      else if (!aboNbOk) desc = `Budget OK (${pct}% sur ${seuil.max}% max), mais ${nb} abonnements c'est beaucoup. Le maximum recommandé est ${MAX_ABO}.`
      else desc = `${pct}% du budget, ${nb} abonnement${nb > 1 ? 's' : ''}. Bonne maîtrise.`

      let tag = ''
      if (!aboBudgetOk) tag = `${pct}% du budget`
      else if (!aboNbOk) tag = `${nb}/${MAX_ABO} recommandés`
      else tag = 'Sous contrôle'
      ...
      results.push({
        ...
        tagType: type,
        tips,
        subs: cat.items.map(d => d.nom),
      })
    } else if (pct > seuil.max) {
      const montantCible = Math.round(total * seuil.max / 100)
      const potentiel = Math.round(cat.montant - montantCible)
      const depassement = pct - seuil.max
      const isAlert = depassement >= 10
      results.push({
        id: cat.nom,
        type: isAlert ? 'danger' : 'warning',
        titre: `${cat.nom} — ${pct}% du budget`,
        desc: `Seuil recommandé : ${seuil.max}%. Vous dépensez ${fmt(cat.montant)}/mois, soit ${fmt(potentiel)} de plus que conseillé. Ramener à ${fmt(montantCible)}/mois libérerait ${fmt(potentiel)} chaque mois.`,
        potentiel,
        tag: `${pct}% du budget`,
      ...
      results.push({
        ...
        desc: `${pct}% du budget (limite ${seuil.max}%). Rien à optimiser ici.`,
        potentiel: 0,
        tag: 'Sous contrôle',
        tagType: 'ok',
      })
...
.op-empty-state-title { font-size: 14px; font-weight: 600; color: var(--muted-foreground); }
.op-empty-state-sub { font-size: 12px; color: var(--muted-foreground); opacity: 0.7; max-width: 260px; line-height: 1.5; }
...
.op-bravo-body { display: flex; flex-direction: column; gap: 3px; }
.op-bravo-title { font-size: 15px; font-weight: 600; color: #14532D; }
.op-bravo-sub { font-size: 12px; color: #166534; line-height: 1.5; }
...
.op-item-icon--danger  { background: #FEE2A0; color: #D97706; }
...
/* Chips abonnements */
.op-subs { display: flex; flex-wrap: wrap; gap: 4px; margin: 6px 0 4px; }
.op-sub-chip {
...
.op-tips li {
  font-size: 11px; color: var(--muted-foreground); line-height: 1.4;
...
.op-tag--danger  { background: #FEF3C7; color: #92400E; }
.op-tag--warning { background: #FFF7ED; color: #C2410C; }
