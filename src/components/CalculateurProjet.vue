    ...
    <template v-if="montantProjet > 0">
      ...
      <div v-if="totalEpargneCouple > 0" class="cp__info-bloc">
        ...
        <div class="cp__info-text">
          ...
          <span v-if="!isSolo" class="cp__info-detail">
      ...
      <div v-else class="cp__results">
        ...
        <template v-if="isSolo">
          <div class="cp__row">
            ...
            <span class="cp__row-label">
              Il reste <strong>{{ fmt(resteP1) }}</strong> à épargner
            </span>
            <span v-if="resteP1 === 0" class="cp__row-done">Objectif atteint ✓</span>
            <span v-else-if="epargneP1 > 0" class="cp__row-dur cp__row-dur--green">
              {{ fmtDuree(resteP1 / epargneP1) }}
              <span class="cp__row-date">atteint en {{ fmtDateCible(resteP1 / epargneP1) }}</span>
            </span>
        ...
        <template v-else>
          ...
          <div class="cp__row">
            ...
            <span v-if="resteP1 === 0" class="cp__row-done">Objectif atteint ✓</span>
            <span v-else-if="epargneP1 > 0" class="cp__row-dur">
              {{ fmtDuree(resteP1 / epargneP1) }}
              <span class="cp__row-date">atteint en {{ fmtDateCible(resteP1 / epargneP1) }}</span>
            </span>
            <span v-else class="cp__row-err">Dépenses supérieures aux revenus</span>
          ...
          <div class="cp__row">
            ...
            <span v-else-if="epargneP2 > 0" class="cp__row-dur">
              {{ fmtDuree(resteP2 / epargneP2) }}
              <span class="cp__row-date">atteint en {{ fmtDateCible(resteP2 / epargneP2) }}</span>
            </span>
            <span v-else class="cp__row-err">Dépenses supérieures aux revenus</span>
          </div>
          <!-- Ensemble -->
          <div class="cp__row cp__row--couple">
            ...
            <span v-else-if="epargneCouple > 0" class="cp__row-dur cp__row-dur--green">
              {{ fmtDuree(resteCouple / epargneCouple) }}
              <span class="cp__row-date">atteint en {{ fmtDateCible(resteCouple / epargneCouple) }}</span>
            </span>
...
function charger(val) { montantProjet.value = val }

// Reset au changement de foyer
watch(() => store.foyerActifId, () => { montantProjet.value = null })

defineExpose({ montantProjet, charger })
...
function fmtDateCible(mois) {
  if (!isFinite(mois) || mois <= 0) return null
  const d = new Date()
  d.setMonth(d.getMonth() + Math.ceil(mois))
...
.cp__row-date { font-size: 11px; font-weight: 400; color: var(--muted-foreground); }
